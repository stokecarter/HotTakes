...
//    @IBOutlet weak var noOfPeoplesLabel: UILabel!
    ...
    var setRooms:Int = 0 {
        didSet{
            let str = "\(setRooms) Rooms Available"
            noOfRoomsLabel.attributedText = attributedText(withString: str, boldString: "\(setRooms)", font: AppFonts.Regular.withSize(12),color: AppColors.themeColor)
        }
    }
    var setPeople:Int = 0 {
        didSet{
...
//            noOfPeoplesLabel.attributedText = attributedText(withString: str, boldString: "\(setRooms)", font: AppFonts.Regular.withSize(12))
    ...
    func populateData(_ event:Event){
        isLive = event.isLive
        eventTimeLabel.text = event.startDateTimeFormat
        imView.setImageWithIndicator(with: URL(string: event.squareImage), placeholderImage: UIImage())
        eventCategoryLabel.text = event.category.name
        eventNameLabel.text = event.name
        setRooms = event.noOfChatrooms
