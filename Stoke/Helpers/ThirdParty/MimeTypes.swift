...
public struct MimeType {
    ...
    static func extValue(for mimeType: String) -> String? {
        
        let first = mimeTypes.first(where: { (dictValue) -> Bool in
            return dictValue.value == mimeType.lowercased()
        })
        
        return first?.key
    }

    static func mimeValue(for ext: String) -> String? {
        
        let first = mimeTypes.first(where: { (dictValue) -> Bool in
            return dictValue.key == ext.lowercased()
        })
